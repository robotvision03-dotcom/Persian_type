...
class ArchiveAndPersistenceTests(unittest.TestCase):
    def test_finished_inspection_is_kept_with_owner_contact(self):
        ...
        with tmp:
            ...
            vehicle = next(item for item in svc.list_vehicles(path) if item["appointment_id"] == appt["id"])
            started = svc.start_inspection(vehicle["id"], db_path=path, now=when)
            self.assertEqual(started["customer_name"], "علی رضایی")
            self.assertEqual(started["customer_phone"], "09120000000")
            early = svc.list_inspected_vehicles(path)
            self.assertEqual(len(early), 1)
            self.assertEqual(early[0]["customer_phone"], "09120000000")
            self.assertFalse(early[0]["finished"])
            saved = svc.finalize_inspection(vehicle["id"], "سالم", db_path=path, now=when)
            self.assertTrue(saved["inspection_completed"])
            self.assertTrue(svc.list_inspected_vehicles(path)[0]["finished"])
